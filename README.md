...
3. 临时文件会显示选中内容；在 `---下面为用户输入内容---` 分隔线下方填写补充指令
4. 点击状态栏 **$(send) 发送给 AIBP** 按钮

同一时间只能保留一个输入文档；再次执行发送命令会聚焦现有草稿，不会创建新标签。
...
- **选区与指令分离** — 临时文件保留选中内容供查看；`---下面为用户输入内容---` 下方的多行 Markdown 作为补充指令发送
- **选区感知** — 自动携带文件绝对路径和光标/选区位置信息
